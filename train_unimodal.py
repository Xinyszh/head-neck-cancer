...
from module.util import get_model, get_optimizer
import argparse
import random
import numpy as np
...
def train():
    ...
    parser.add_argument('--exp_name', type=str, default='T1-COR-18-before', help="name of this experiment")
    parser.add_argument('--seed', type=int, default=1, help="1, 2, 3, or any other seeds")
    parser.add_argument('--model_tag', type=str, default='ResNet18', help="Choose a model.")
    parser.add_argument('--optimizer_tag', type=str, default='Adam', help="Choose a optimizer.")
    parser.add_argument('--modal', type=str, default='T1_COR_before', help="Choose modal for experiment.")
    parser.add_argument('--bs', type=int, default=7, help="Batch size.")
    ...
    parser.add_argument('--device', type=int, default=2, help="Choose gpu device.")
    parser.add_argument('--device_gpus', type=list, default=[1,5, 6], help="Choose gpu device.")
    parser.add_argument('--log_dir', type=str, default='/jhcnas1/xinyi/head_neck_cancer/split',
                        help="Address to store the log files.")
    ...
    if not args.debug:
        ...
        shutil.copyfile('./scripts/train_uni.sh', os.path.join(log_dir, "bk", exp_name, './train_uni.sh'))
    ...
    num_classes = 2
    data_size = None
    if args.modal == 'T1_COR_concate':
        data_size = (2, 45, 384, 384)
    elif args.modal == 'T1_TRA_concate':
        data_size = (2, 70, 384, 384)
    elif args.modal == 'T2_TRA_concate':
        data_size = (2, 36, 384, 384)
    elif args.modal=='T1_COR_before':
        data_size = (1, 45, 384,384)
    elif args.modal == 'T1_TRA_before':
        data_size = (1, 70, 384, 384)
    elif args.modal == 'T2_TRA_before':
        data_size = (1, 36, 384, 384)
    elif args.modal=='T1_COR_after':
        data_size = (1, 45, 384, 384)
    elif args.modal == 'T1_TRA_after':
        data_size = (1, 70, 384, 384)
    elif args.modal == 'T2_TRA_after':
        data_size = (1, 36, 384, 384)
    model = get_model(args.model_tag, num_classes, data_size)
    ...
    def eval(model, data_loader):
        ...
        for index, data, label in tqdm(data_loader, leave=False):
            data = data.float()
            data = data.to(device)
    ...
    for epoch in range(args.num_epochs):
        for iter_num, (_, data, label) in tqdm(enumerate(train_loader)):
